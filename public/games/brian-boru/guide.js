import {setDisclosureExpanded} from '../../disclosure.js';
...
const asset = name => new URL(`../../guide-assets/brian-boru/${name}`, import.meta.url).href;
...
function renderTable(table) {
  ...
  return `<div class="table-scroll" tabindex="0" role="region" aria-label="${escape(labels.table)}"><table>
    ...
    <tbody>${table.rows.map((row, rowIndex) => `<tr>${row.map((cell, cellIndex) => `<td>${cellIndex === 0 && table.icons?.[rowIndex] ? `<img class="rule-symbol" src="${asset(table.icons[rowIndex])}" width="${imageSizes[table.icons[rowIndex]][0]}" height="${imageSizes[table.icons[rowIndex]][1]}" alt="" />` : ''}${escape(cell)}</td>`).join('')}</tr>`).join('')}</tbody>
  </table></div>`;
}
...
function renderRules(topic) {
  const body = `<div class="topic-rules">
    ...
    ${(topic.details ?? []).map(detail => {
      ...
      return ['tribes', 'gadgets', 'solo-variants'].includes(topic.id)
...
function renderExamples(topic) {
  return examplesFor(topic, language).map(example => `<figure class="rule-example">
    <a href="${asset(example.image)}" target="_blank" rel="noopener">
      <img src="${asset(example.image)}" alt="${escape(example.caption)}" loading="lazy" width="${imageSizes[example.image][0]}" height="${imageSizes[example.image][1]}" style="max-width: min(100%, ${Math.min(560, Math.max(150, imageSizes[example.image][0] / 2))}px)" />
    </a>
...
const chat = createRuleChat({
  slug: 'brian-boru', log: $('#chat-log'), form: $('#chat-form'),
...
function renderAll() {
  const checkpointState = new Map([...document.querySelectorAll('.checkpoint')].map(node => [node.dataset.checkId, node.classList.contains('is-collapsed')]));
